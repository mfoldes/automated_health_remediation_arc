...
"@
    ...
    # .NET Framework (Windows PowerShell 5.1); .NET 6+ removed it.
    # On PowerShell 7+ we create an empty file with CreateNew,
    # immediately apply the restricted ACL via Set-Acl, then write the
    # secret bytes. Because no secret bytes are written until AFTER
    # the ACL is narrowed, the secret never touches disk under a
    # default ACL. The temp file lives in a process-local directory
    # with a random GUID name, so the brief empty-file window between
    # CreateNew and Set-Acl is not exploitable.
